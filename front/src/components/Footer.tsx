...
export default function Footer({}: Props) {
  ...
  return (
    <View className='fixed bottom-0 left-0 right-0 flex justify-between items-center bg-white py-2 border-t border-gray-200 h-[5%]'>
      <View className='flex-1 flex flex-col items-center' onClick={() => Taro.switchTab({ url: '/pages/dashboard/index' })}>
        <Text className={`iconfont icon-home text-xl ${isActive('dashboard') ? 'text-[#fbb713]' : ''}`}></Text>
        <Text className={`text-base mt-1 ${isActive('dashboard') ? 'text-[#fbb713]' : ''}`}>首页</Text>
      </View>
      <View className='flex-1 flex flex-col items-center' onClick={() => Taro.switchTab({ url: '/pages/products/index' })}>
      ...
      <View className='flex-1 flex flex-col items-center' onClick={() => Taro.switchTab({ url: '/pages/order/index' })}>
      ...
      <View className='flex-1 flex flex-col items-center' onClick={() => Taro.switchTab({ url: '/pages/setting/index' })}>
        <Text className={`iconfont icon-user text-xl ${isActive('setting') ? 'text-[#fbb713]' : ''}`}></Text>
        <Text className={`text-base mt-1 ${isActive('setting') ? 'text-[#fbb713]' : ''}`}>我的</Text>
      </View>
    </View>
  )
}