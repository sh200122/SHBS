...
function genResource(path, pages, loaderContext, syncFileName = false) {
    ...
    const stringify = (s) => (0, util_1.stringifyRequest)(loaderContext, s);
    const importDependent = syncFileName ? 'require' : 'import';
    return `Object.assign({
  path: '${path}',
  load: function(context, params) {
    const page = ${importDependent}(${stringify((0, node_path_1.join)(loaderContext.context, syncFileName || path))})
    return [page, context, params]
  }
}, ${JSON.stringify((0, helper_1.readConfig)(pages.get(path.split(node_path_1.sep).join('/')), options))})`;
}
function default_1() {
    var _a, _b;
    ...
    const pathDirname = (0, node_path_1.dirname)(this.resourcePath);
    const pageName = isMultiRouterMode ? (0, node_path_1.join)(pathDirname, options.filename).replace(options.sourceDir + node_path_1.sep, '') : '';
    ...
    if (isBuildNativeComp) {
        ...
        const compPath = (0, node_path_1.join)(pathDirname, options.filename);
...
export default nativeComponent`;
        ...
        const mockDataPath = (0, node_path_1.join)(this.context, mockDataRelativePath);
        const hasMock = helper_1.fs.existsSync(mockDataPath);
        code += `
...
document.title = config.navigationBarTitleText || ''`;
    ...
    if (options.bootstrap)
        return `import(${stringify((0, node_path_1.join)(options.sourceDir, `${isMultiRouterMode ? pageName : options.entryFileName}.boot`))})`;
...
`;
    if (config.tabBar) {
        ...
        for (let i = 0; i < tabbarList.length; i++) {
            ...
            if (t.iconPath) {
                const iconPath = stringify((0, node_path_1.join)(pathDirname, t.iconPath));
            ...
            if (t.selectedIconPath) {
                const iconPath = stringify((0, node_path_1.join)(pathDirname, t.selectedIconPath));
...
import component from ${stringify((0, node_path_1.join)(options.sourceDir, options.entryFileName))}
