...
class MiniSplitChunksPlugin extends SplitChunksPlugin_1.default {
            ...
            compilation.hooks.processAssets.tapAsync({
                ...
                for (const entryName of compilation.entries.keys()) {
                    if (this.isSubEntry(entryName)) {
                        ...
                        if (node_path_1.default.extname(entryName))
                            continue;
                        ...
                        for (const key in FileExtsMap) {
                            ...
                            if (ext === FileExtsMap.JS || ext === FileExtsMap.STYLE || ext === FileExtsMap.TEMPLATE) {
                                ...
                                const chunkAbsolutePath = node_path_1.default.resolve(this.distPath, chunkName);
                                const subVendorsPath = node_path_1.default.join(subRoot, `${SUB_VENDORS_NAME}${ext}`);
                                // 将子包 vendors 插入到 entry 中
                                if (assets[(0, helper_1.normalizePath)(subVendorsPath)]) {
                                    const subVendorsAbsolutePath = node_path_1.default.resolve(this.distPath, subVendorsPath);
                                ...
                                subCommon.forEach(moduleName => {
                                    ...
                                    const moduleFilePath = node_path_1.default.join(SUB_COMMON_DIR, moduleFileName);
                                    const subRootModuleFilePath = node_path_1.default.join(subRoot, moduleFilePath);
                                    const assetSource = assets[(0, helper_1.normalizePath)(moduleFilePath)];
                                    if (assetSource) {
                                        const moduleAbsolutePath = node_path_1.default.resolve(this.distPath, subRootModuleFilePath);
                                        const chunkRelativePath = this.getRealRelativePath(node_path_1.default.resolve(this.distPath, chunkName), moduleAbsolutePath);
                                        ...
                                        const originSourceMapPath = node_path_1.default.join(SUB_COMMON_DIR, `${moduleName}${FileExtsMap.JS_MAP}`);
                                        const originSourceMap = assets[originSourceMapPath];
                                        // 输出 source map
                                        if (ext === FileExtsMap.JS && originSourceMap) {
                                            const subRootSourceMapFilePath = node_path_1.default.join(subRoot, originSourceMapPath);
    ...
    matchSubVendors(chunk) {
        const subVendorsRegExps = this.subRoots.map(subRoot => new RegExp(`^${(0, helper_1.normalizePath)(node_path_1.default.join(subRoot, SUB_VENDORS_NAME))}$`));
    ...
    getSubPackageVendorsCacheGroup() {
        ...
        this.subRoots.forEach(subRoot => {
            subPackageVendorsCacheGroup[subRoot] = {
                ...
                name: (0, helper_1.normalizePath)(node_path_1.default.join(subRoot, SUB_VENDORS_NAME)),
                minChunks: 2,
                priority: 10000
            };
        });
    ...
    getSubCommonCacheGroup() {
        ...
        this.subCommonDeps.forEach((depInfo, depName) => {
            const cacheGroupName = (0, helper_1.normalizePath)(node_path_1.default.join(SUB_COMMON_DIR, depName));
    ...
    getRealRelativePath(from, to) {
        return (0, helper_1.promoteRelativePath)(node_path_1.default.relative(from, to));
    }
    /**
